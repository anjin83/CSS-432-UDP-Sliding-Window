...
int main( int argc, char *argv[] ) {
  ...
  if ( myPart == CLIENT ) {
	...
	cerr << "retransmits = " << retransmits << endl;
      ...
      break;
    case 4:
    for(int i = 1; i <= 10; i++)	// run the test for 1-10 percent packet drops 
    {
	      for ( int windowSize = 1; windowSize <= MAXWIN; windowSize+=29 ) {
		timer.start( );                                        // start timer
		retransmits =
		clientSlidingWindow( sock, MAX, message, windowSize, dropPercent ); // actual test
		cerr << "Percentage packet dropped = ";                              // lap timer
		cout << dropPercent << " ";
		cerr << "Elasped time = "; 
		cout << timer.lap( ) << endl;
		cerr << "retransmits = " << retransmits << endl;
  ...
  if ( myPart == SERVER ) {
    ...
    {
      for ( int windowSize = 1; windowSize <= MAXWIN; windowSize+= 29 )
	serverEarlyRetrans( sock, MAX, message, windowSize, i );
    }
      break;
    default:
      cerr << "no such test case" << endl;
...
int clientSlidingWindow( UdpSocket &sock, const int numSends, int message[], 
	...
	while(cumack < numSends-1){
		...
		while((lastPacketSent - cumack) <= windowSize && lastPacketSent < numSends){
			message[0] = lastPacketSent++;
			sock.sendTo( ( char * )message, MSGSIZE );
		...
		while(1){
			...
			else if(theTimer.lap() > 1500){
				counter += (lastPacketSent - cumack);		
...
// Test 4: client sliding window message send with packet drops--------------------
...
int clientSlidingWindow( UdpSocket &sock, const int numSends, int message[], 
	...
	while(dropRate == -1)
	{
		...
		sock.recvFrom( ( char * ) &dropRate, sizeof(dropRate));
		drop = dropRate;
	}
	int clientAck = -1;
	...
	{
		theTimer.start();
		sock.ackTo((char *) &dropRate, sizeof(dropRate));
		while(sock.pollRecvFrom() <= 0)
		{
			if(theTimer.lap() > 1500)
			{   
...
// Test4: server sliding window message receive with packet drops------------------

void serverEarlyRetrans( UdpSocket &sock, const int max, int message[], 
             int windowSize, int packetDrop) {//packetDrop is the percentage of packets to drop.
    cerr << "server sliding window test:" << endl;
    Timer theTimer;
    bool ackArray[max];
    for(int i =0; i < max; i++)
    ...
    while(cumAck < max)
    {
        if(sock.pollRecvFrom() > 0)
        {
            ...
            int currentPacket = message[0];
	          int r = ((rand()%100)+1);
            if((r > packetDrop && currentPacket == cumAck) || currentPacket == 0 ) //drop percentToDrop packets
    	      {
          		message[0] = cumAck;	
              		sock.ackTo( (char *)message, MSGSIZE );
    		     cumAck++;
    ...
    while(clientReceived == -1)
    {
        sock.ackTo( (char *)&packetDrop, sizeof(packetDrop));
	      theTimer.start();
        while(sock.pollRecvFrom() <= 0)  //client sent an ack
        {
            if(theTimer.lap() > 1500)
